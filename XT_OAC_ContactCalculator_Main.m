...
%        <Item name="Contact Calculator - Main" icon="Matlab">
%          <Command>MatlabXT::XT_OAC_ContactCalculator_Main(%i)</Command>
%        </Item>
%       </Submenu>
%      </Menu>
%      <SurpassTab>
...
%This version of Contact Calculator is not suitable for use if the image 
...
function XT_OAC_ContactCalculator_Main(aImarisApplicationID)
...
for PrimIter = 1:length(vPrimarySurface)
    ...
    for q=1:g
        ...
        for vIndexZ = vStartZ:vEndZ
            %Mask for secondary Surface (inside=1, outside=0)
            ...
            if nnz(vSlice2)==0
                ...
                ch2 = ch1<UpperThreshold;
                %ch1 is from the distance transform channel for outside the
                %primary surface
                %ch2 has a value of 1 everywhere that the distance
                %transform channel value is less than UpperThreshold and a
                %value of 0 everywhere else
                vSlice2 = vSlice2 == 1;% for inside secondary surface only
                vData=(vSlice2.*ch2); %each voxel inside the secondary surface but less than UpperThreshold distance from the primary surface is 1; all other voxels are 0
                vResultValues(q) = vResultValues(q)+sum(vData,'all');
            end
        end
    ...
    clear vResultValues vSurfaceContact vAllSurfaceArea vSecondarySurface vSecondaryMask vMaskDataSetPrimary vSlice vSlice2 vData ch1 ch2 
    ...
    clear filenameprep filename fileID r s t u q h g vProgressDisplay vIndexX vIndexY vIndexZ vMinDistanceValue 
    clear vPrimLowerZ vPrimUpperZ vStartZ vEndZ vSizeZ vPrimLowerX vPrimUpperX vStartX vEndX vSizeX vPrimLowerY vPrimUpperY vStartY vEndY vSizeY 
end
